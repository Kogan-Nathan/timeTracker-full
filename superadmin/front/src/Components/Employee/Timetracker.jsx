...
import RowReport from './Mapping/RowReport'
import LoadingSpinner from '../General-Components&Pages/LoadingSpinner'
...
export default function Timetracker(props) {
    ...
    async function getUser(id){
        await fetch('http://localhost:9000/users', {headers: {'company-name' : props.company}})
        .then(response=> response.json())
            ...
            return(
                <div className="container">
                    <div className="tableConatainer tableProjects">
                        ...
                        <div className="grid-userNewReport" style={{width:"100%", marginBottom:"20px"}}>
                            <select className="inputTime select1" onChange={(e)=>setProjectName(e.target.value)} required>
                                ...
                                {Projects.map( value =>{
                                    return <option key={value.projectID}>{value.projectName}</option>
                                })}
                            </select>
                    ...
                    <div className="scroll">
                        {reports.map( value =>{return <RowReport key={value.Id} report={value}/>})}
                    </div></div> : <h4> Go ahead, submit your first report! </h4>}
