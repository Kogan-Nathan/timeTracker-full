...
export default function SpesificTime() {
    ...
    const onlySpesificTime = (reportArray) =>{
        if(toDate&&fromDate){
            if(fromDate < toDate){
                ...
                if(reportArray.length === 0){
                    document.getElementById("title").innerHTML="no reports found..."
                }
                setShow(true)
            }
            if(fromDate > toDate){
    ...
    return (
        <div className="container">
            ...
            {reportForSpesific.length>0? <div>
            ...
            <div className="scroll">
                {show && reportForSpesific.map( value =>{return <RowSummary key={value.Id} report={value}/>})}
            </div>
            <hr className="hrBorder"/>
            </div> : <h4 id="title">Please select a time period</h4>}
        </div>
    )
}